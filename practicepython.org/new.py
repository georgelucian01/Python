...
def get_next_script_number(folder):
    ...
    script_files = [f for f in file_list if f.startswith("python_practice_") and f.endswith(".py")]

    # Extract the numbers from the filenames and find the maximum
    numbers = [int(f[len("python_practice_"):-len(".py")]) for f in script_files]
...
if __name__ == "__main__":
    ...
    folder_path = "."  # Saves directly to script location
    next_script_number = get_next_script_number(folder_path)
    new_script_file_name = f"python_practice_{next_script_number}.py"
